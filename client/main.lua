...
local function MenuGarage()
    ...
    for k in pairs(Config.Vehicles) do
        towMenu[#towMenu+1] = {
            ...
            params = {
                event = "kg-tow:client:TakeOutVehicle",
                args = {
                    vehicle = k
                }
            }
    ...
    exports['kg-menu']:openMenu(towMenu)
end
...
local function CreateZone(type, number)
    ...
    elseif type == "towspots" then
        event = "kg-tow:client:SpawnNPCVehicle"
    ...
    if Config.UseTarget and type == "main" then
        exports['kg-target']:AddBoxZone(boxName, coords, size, size, {
            minZ = coords.z - 5.0,
            maxZ = coords.z + 5.0,
            name = boxName,
            heading = heading,
        ...
        zoneCombo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if type == "main" then
                    TriggerEvent('kg-tow:client:PaySlip')
                elseif type == "vehicle" then
                    TriggerEvent('kg-tow:client:Vehicle')
                elseif type == "towspots" then
                    TriggerEvent('kg-tow:client:SpawnNPCVehicle')
                end
            end
        end)
        if type == "vehicle" then
            ...
            zoneComboV:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    TriggerEvent('kg-tow:client:ShowMarker', true)
                else
                    TriggerEvent('kg-tow:client:ShowMarker', false)
                end
            end)
        elseif type == "towspots" then
...
local function deliverVehicle(vehicle)
    ...
    KGCore.Functions.Notify(Lang:t("mission.delivered_vehicle"), "success")
    KGCore.Functions.Notify(Lang:t("mission.get_new_vehicle"))
...
RegisterNetEvent('kg-tow:client:SpawnVehicle', function()
...
RegisterNetEvent('KGCore:Client:OnPlayerLoaded', function()
    PlayerJob = KGCore.Functions.GetPlayerData().job
...
RegisterNetEvent('jobs:client:ToggleNpc', function()
    if KGCore.Functions.GetPlayerData().job.name == "tow" then
        if CurrentTow ~= nil then
            KGCore.Functions.Notify(Lang:t("error.finish_work"), "error")
            return
        end
        NpcOn = not NpcOn
        if NpcOn then
...
RegisterNetEvent('kg-tow:client:TowVehicle', function()
    ...
    if isTowVehicle(vehicle) then
        if CurrentTow == nil then
            ...
            if NpcOn and CurrentLocation then
                if GetEntityModel(targetVehicle) ~= joaat(CurrentLocation.model) then
                    KGCore.Functions.Notify(Lang:t("error.vehicle_not_correct"), "error")
                    return
                end
            end
            if not IsPedInAnyVehicle(PlayerPedId()) then
                if vehicle ~= targetVehicle then
                    ...
                    if #(towPos - targetPos) < 11.0 then
                        KGCore.Functions.Progressbar("towing_vehicle", Lang:t("mission.towing_vehicle"), 5000, false, true, {
                            ...
                            if NpcOn then
                                RemoveBlip(CurrentBlip)
                                KGCore.Functions.Notify(Lang:t("mission.goto_depot"), "primary", 5000)
                                ...
                                TriggerServerEvent('kg-tow:server:nano', vehNetID)
                                --remove zone
                                CurrentLocation.zoneCombo:destroy()
                            end
                            KGCore.Functions.Notify(Lang:t("mission.vehicle_towed"), "success")
                        end, function() -- Cancel
                            StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
                            KGCore.Functions.Notify(Lang:t("error.failed"), "error")
                        end)
                    end
                end
            end
        else
            KGCore.Functions.Progressbar("untowing_vehicle", Lang:t("mission.untowing_vehicle"), 5000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                CurrentTow = nil
                drawDropOff = false
                KGCore.Functions.Notify(Lang:t("mission.vehicle_takenoff"), "success")
            end, function() -- Cancel
                StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
                KGCore.Functions.Notify(Lang:t("error.failed"), "error")
            end)
        ...
        KGCore.Functions.Notify(Lang:t("error.not_towing_vehicle"), "error")
    end
end)

RegisterNetEvent('kg-tow:client:TakeOutVehicle', function(data)
    local coords = Config.Locations["vehicle"].coords
    ...
    if #(pos - coords) <= 5 then
        ...
        TriggerServerEvent('kg-tow:server:DoBail', true, vehicleInfo)
...
RegisterNetEvent('kg-tow:client:Vehicle', function()
    local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
    if not CurrentTow then
        if vehicle and isTowVehicle(vehicle) then
            DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
            TriggerServerEvent('kg-tow:server:DoBail', false)
        else
            MenuGarage()
        end
    else
...
RegisterNetEvent('kg-tow:client:PaySlip', function()
    if JobsDone > 0 then
        RemoveBlip(CurrentBlip)
        TriggerServerEvent("kg-tow:server:11101110", JobsDone)
        JobsDone = 0
        NpcOn = false
    else
        KGCore.Functions.Notify(Lang:t("error.no_work_done"), "error")
    end
end)

RegisterNetEvent('kg-tow:client:SpawnNPCVehicle', function()
...
RegisterNetEvent('kg-tow:client:ShowMarker', function(active)
    if PlayerJob.name == "tow" then
        showMarker = active
    end
end)
