...
RegisterNetEvent('kg-tow:server:DoBail', function(bool, vehInfo)
    local src = source
    ...
    if bool then
        if Player.PlayerData.money.cash >= Config.BailPrice then
            ...
            TriggerClientEvent('KGCore:Notify', src, Lang:t('success.paid_with_cash', { value = Config.BailPrice }), 'success')
            TriggerClientEvent('kg-tow:client:SpawnVehicle', src, vehInfo)
        elseif Player.PlayerData.money.bank >= Config.BailPrice then
            Bail[Player.PlayerData.citizenid] = Config.BailPrice
            Player.Functions.RemoveMoney('bank', Config.BailPrice, 'tow-paid-bail')
            TriggerClientEvent('KGCore:Notify', src, Lang:t('success.paid_with_bank', { value = Config.BailPrice }), 'success')
            TriggerClientEvent('kg-tow:client:SpawnVehicle', src, vehInfo)
        else
            TriggerClientEvent('KGCore:Notify', src, Lang:t('error.no_deposit', { value = Config.BailPrice }), 'error')
        end
    else
        if Bail[Player.PlayerData.citizenid] ~= nil then
            Player.Functions.AddMoney('bank', Bail[Player.PlayerData.citizenid], 'tow-bail-paid')
            Bail[Player.PlayerData.citizenid] = nil
            TriggerClientEvent('KGCore:Notify', src, Lang:t('success.refund_to_cash', { value = Config.BailPrice }), 'success')
        end
    end
end)

RegisterNetEvent('kg-tow:server:nano', function(vehNetID)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    ...
    if chance < 26 then
        exports['kg-inventory']:AddItem(src, 'cryptostick', 1, false, false, 'kg-tow:server:nano')
        TriggerClientEvent('kg-inventory:client:ItemBox', src, KGCore.Shared.Items['cryptostick'], 'add')
    end
end)

RegisterNetEvent('kg-tow:server:11101110', function(drops)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    if not Player then return end
    local playerPed = GetPlayerPed(src)
    ...
    TriggerClientEvent('KGCore:Notify', src, Lang:t('success.you_earned', { value = payment }), 'success')
end)

KGCore.Commands.Add('npc', Lang:t('info.toggle_npc'), {}, false, function(source)
    TriggerClientEvent('jobs:client:ToggleNpc', source)
end)

KGCore.Commands.Add('tow', Lang:t('info.tow'), {}, false, function(source)
    ...
    if Player.PlayerData.job.name == 'tow' or Player.PlayerData.job.name == 'mechanic' then
        TriggerClientEvent('kg-tow:client:TowVehicle', source)
    end
end)
